...
const scriptSchema = {
  ...
  properties: {
    ...
    tone: { type: Type.STRING, description: "The overall tone or mood of the story (e.g., 'dark and gritty', 'lighthearted and comedic')." },
    themes: {
      type: Type.ARRAY,
      items: { type: Type.STRING },
    ...
    setting: {
      type: Type.OBJECT,
      properties: {
        time: { type: Type.STRING, description: "The time period in which the story is set (e.g., 'Present Day', '2042')." },
        location: { type: Type.STRING, description: "The primary physical location of the story (e.g., 'A remote space station')." },
      },
    ...
    characters: {
      ...
      items: {
        ...
        properties: {
          ...
          role: { type: Type.STRING, description: "The character's primary role in the story (e.g., 'Protagonist', 'Mentor', 'Antagonist')." },
          description: { type: Type.STRING, description: "A brief description of the character's personality, appearance, and motivations." },
        },
          ...
          scenes: {
            ...
            items: {
              ...
              properties: {
                ...
                visual_style: { type: Type.STRING, description: "The visual aesthetic for the scene (e.g., 'High-contrast noir lighting', 'Sweeping desert vistas')." },
                audio_style: { type: Type.STRING, description: "The auditory style for the scene (e.g., 'Tense, orchestral score', 'Ambient, futuristic sounds')." },
                dialogues: {
                  type: Type.ARRAY,
                  items: {
                    type: Type.OBJECT,
                    properties: {
                      role: { type: Type.STRING, description: "The role of the character speaking the line (e.g., 'Protagonist', 'Mentor'). Must match a role from the characters list." },
...
export const generateScript = async (prompt: string, language: 'en-US' | 'vi-VN', apiKey: string): Promise<Root> => {
  try {
    ...
    throw new Error(`Không thể tạo kịch bản. Vui lòng kiểm tra khóa API và kết nối mạng của bạn. Chi tiết: ${error instanceof Error ? error.message : String(error)}`);
  }
};
...
export const suggestPlotPoints = async (prompt: string, language: 'en-US' | 'vi-VN', apiKey: string): Promise<string[]> => {
    try {
        const ai = getAiClient(apiKey);
...
export const generateSceneImage = async (prompt: string, aspectRatio: AspectRatio, apiKey: string, negativePrompt?: string): Promise<string> => {
    try {
        const ai = getAiClient(apiKey);
        const config = {
            numberOfImages: 1,
...
export const enhanceText = async (textToEnhance: string, context: string, language: 'en-US' | 'vi-VN', apiKey: string): Promise<string> => {
    try {
        const ai = getAiClient(apiKey);
