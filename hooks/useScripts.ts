...
export const useScripts = () => {
    ...
    const updateScriptField = async (path: string, value: any, apiKey?: string | null) => {
        if (!activeScript) return;
        const updatedScript = structuredClone(activeScript);
        setNestedValue(updatedScript, path, value);
        setActiveScript(updatedScript); // Optimistic update
